...
### 1. 型付き source スキーマ（kind = テーブル名、フィールドの存在が判別子）

`type = "..."` の tag 文字列は持たず、**kind をテーブル名そのものにする**。
TOML (config / defs):
...
- エントリ構造体は `command: Option<CommandSpec>` / `op: Option<OpSpec>` / … を並べ、
  「**ちょうど 1 つ**が存在する」を検証する (判別子の重複記述なし、ベンダ追加 =
  optional フィールド 1 個)。各 Spec は `deny_unknown_fields`、
...
- **原形を保持する**: `op` kind は実行時に argv へ**降ろす** (lowering: 既存の内部
  ...
  冪等比較は**型付き形式で**行う。冪等 (b) 規則の完全一致は「kind + 全 kind 別
  フィールド + TTL」の一致。wire も同じ field-presence 形 (`{"command": {...}}` /
  `{"op": {...}}`) を運ぶ。
- CLI: `--command ARGV...` (rest 消費) を argv の入口として維持し、`--source URI` は
  op kind の入口。cwd/env は `--command-cwd` / `--command-env NAME=V` の prefix
  グルーピング (`--otp-*` と同じ手法) で `--command` より前に置く。
  `--command-argv` という別名は作らない (同じものに名前を 2 つ与えない)。
...
- **`source = { type = "command", ... }` の tag 付き封筒形式**
  - 不採用理由 (検討過程の暫定案): `type` 文字列が kind 情報の重複記述になる。
    kind をテーブル名にすれば TOML として自然に読め、判別もフィールドの存在で済む。
