...
```
cache-warden kv prefetch [KEY...] [--namespace NS]... [--defs FILE]...
                         [--all] [--pin DURATION]
```
...
- 位置引数 KEY = 現在 NS のキー
- `--namespace NS` (反復可) = その NS の**全定義**を追加
- `--defs FILE` (反復可) = defs ファイル内の定義を define (冪等、DR-0014 の既存規則)
  した上で、そのファイルに載っている key 集合を対象に追加 —
  **プロジェクト単位のウォームアップ** (`kv prefetch --defs .cache-warden.toml --pin 8h`)
- `--all` = 全 NS の全定義。namespace 指定が既に「その NS の全キー」を意味する以上、
  「全部」のフラグに namespaces を冠する必要はない — **`kv list` / `status` の既存
  `--all-namespaces` も `--all` にリネーム**して統一する (DR-0017 の改訂、pre-1.0
  互換レイヤなし)
