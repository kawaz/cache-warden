...
### 参照構文: `cache-warden://[NS/]KEY`（DR-0017 で改訂）

- スキームは `cache-warden://` の 1 種のみ。短縮形（`cw://` 等）のエイリアスは設けない
  （正規表記を 1 つに保つ。CLI 設計方針「エイリアスを指示なく増やさない」と整合）。
- 参照は `[NS/]KEY`。**未修飾の `KEY` は呼び出し文脈の namespace に解決され、
  `NS/KEY` 修飾は絶対参照**（DR-0017 §3）。
- KEY / NS の文字種は `[A-Za-z0-9_]+`（DR-0017 §1.5 で `.` と `-` を当初の文字種から
  除外し、生成時にプロトコル境界で強制するよう改訂）。文字種に `/` が無いため
  `NS/KEY` の構文上の曖昧さはなく、テンプレート中の参照の終端も一意に決まる。
...
- 参照可能キーに文字種制約が生まれる。DR-0017 §1.5 以降は文字種が `[A-Za-z0-9_]+` に
  縮小され、**生成時にプロトコル境界で強制**されるため「参照できないキー」は存在しない。
...
- [DR-0017-kv-namespaces](./DR-0017-kv-namespaces.md) — 参照文法への `[NS/]` 修飾の追加と KEY 文字種の `[A-Za-z0-9_]+` への改訂
