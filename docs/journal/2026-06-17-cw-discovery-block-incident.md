...
## サイドノート: worktree cleanup で別件発見

本作業の後始末 (= worktree cleanup) で Claude Code `ExitWorktree(remove)` が
dir 削除に失敗、調査の結果 jj-worktree (`kawaz/jj-worktree`) の `.jj-worktree-meta/`
が `EnterWorktree` 経由作成では生成されない件にぶつかった。

部外者 (= cache-warden 側) として 1 度目の起票では「meta auto-backfill しろ」と
焦点ズレして land してしまったが、kawaz 指摘 + 実機検証で `jj workspace root --name <ws>`
や WorkspaceRef.root() Template など jj 標準 API が path を提供している事実が判明、
**meta system 自体が redundant、廃止が筋良い**が正しい結論だった。

差し戻して再起票したのが `kawaz/jj-worktree` の
`docs/issue/2026-06-18-drop-jj-worktree-meta-redundant.md`。本件と直接の因果は
無いが、同セッション中の派生事象として記録。
