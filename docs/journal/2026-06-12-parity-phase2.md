...
| 2.6 TouchID 回数 | **キャッシュ生存中 0 回を実機確認** ✓。初回 fetch も op セッション認可グレース内なら 0 回 (emerada/syun の初回署名は kawaz 離席中に成功 = プロンプトの頻度は op セッションポリシー側が支配、warden と同一の仕組み)。cache-warden 固有の増加要素は hard TTL ごとの再 fetch のみ |
