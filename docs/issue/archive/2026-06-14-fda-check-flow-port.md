...
discard_reason:
pending_reason:
close_reason: ["done:FDA check flow implemented (macos-tcc crate + internal fda-check subcommand + daemon register integration)","followup:2026-07-04-fda-readme-section-cache-warden"]
blocked_by:
origin:
---
...
## 2026-07-04 実装確認 (triage sweep)

コード調査の結果、移植計画の主要項目は既に実装済みと確認:

- `crates/macos-tcc/` workspace crate 追加済み (`Permission` / `AuthState` /
  `check` / `check_via_app_bundle` / `open_settings` / `wait_for_grant` 等の API)
- `internal fda-check` サブコマンド実装済み (`crates/cache-warden-cli/src/commands/internal_cmd.rs`)
- `daemon register` (`commands/daemon_cmd.rs`) に FDA セットアップフローが統合済み:
  `has_op_sources(config)` gate → `check_via_app_bundle` (3 回リトライ) →
  未許可なら `open_settings` + `wait_for_grant` で誘導 → 完了後 System Settings を
  osascript で閉じる (commit 履歴: `feat(macos-tcc): introduce workspace crate`,
  `feat(cache-warden-cli): add internal fda-check subcommand`,
  `feat(cache-warden-cli): integrate FDA setup flow into daemon register`)

未実施なのは **README (ja/en) への「Full Disk Access」節の追加のみ**
(`grep -n "Full Disk Access" README.md README-ja.md` で該当なし)。実装は完了
しているため本 issue は resolved で close し、README 追記は別 issue で追跡する。
