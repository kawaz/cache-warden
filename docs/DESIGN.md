...
- **Value source**: The supplier of the value. Two runtime kinds (plus typed kinds at the definition layer, DR-0018):
  ...
  - `command`: The output of an upstream command. Registered as a definition via `define` (using `source = "command"` with `command.argv`, or `source = "op"` with `op.uri` — the typed original is preserved; the `op` kind is lowered to a command invocation at runtime). Produced lazily on the first get. Can be regenerated by re-running the command after hard TTL expiry.
...
- **Commands v1**: `ping` / `status` (daemon info + entry list; **no values**; also lists "definition-only (no
  ...
  `kv.define` (key + typed source `{"source": "command", "command": {...}} | {"source": "op", "op": {...}}` + soft/hard TTL seconds: registers the definition of a
  value-sourced key only and does not run the upstream; exact-match no-op / mismatch `bad_request`; DR-0014 / DR-0018) /
...
- **Peer authentication**: per connection, the peer pid is obtained via LOCAL_PEERPID (macOS) / SO_PEERCRED (Linux),
  ...
  interpretation yet**).
- **Re-authentication**: config-driven (DR-0010 / DR-0018). `[auth]` omitted = `AllowAll`. When
  present, `type` is required. `type = "command"` runs a `CommandAuthenticator` (delegating to an
  external command, exit 0 = approved). Future kinds (`touchid`, `push`, etc.) slot in behind the
  same `Authenticator` trait without changing the shape of existing config.
...
registers a value-sourced key's definition (the upstream is not run — deferred to the first get; idempotent =
exact-match no-op / mismatch error). `--command ARGV...` is sugar for `source = "command"` + `command.argv`;
`--source op://URI` is sugar for `source = "op"` + `op.uri` (the typed original is preserved in the definition —
the op URI is visible in status and used for idempotency comparison; anything other than op:// is an error).
`--command-cwd` / `--command-env` (repeatable) set `command.cwd` / `command.env` and must appear before `--command`.
...
### Configuration (TOML config + re-auth command, DR-0010 / DR-0018)
...
Typed source / auth schemas are defined in [DR-0018](./decisions/DR-0018-typed-sources-auth-and-prefetch.md).
...
[auth]
type = "command"                       # omitted => AllowAll (no re-auth). type is required when [auth] is present. DR-0018
command = ["/path/to/reauth-prompt"]

[kv.DB_PASSWORD]                       # definition registered at startup; execution is lazy until the first get
source = "command"
command.argv = ["op", "read", "op://vault/item/password"]
soft-ttl = "1h"
hard-ttl = "24h"

[kv.MY_OP_SECRET]                      # op kind: stores the typed original (uri visible in status); lowered to op-read at runtime
source = "op"
...
- **No inline static values**: a `[kv.*]` entry must carry a `source` field (`"command"` or `"op"`)
  with its kind-specific sub-fields. Writing a literal secret value (`value`, etc.) is a
  configuration error — this structurally prevents a plaintext secret from being persisted in
  config. Literal values are piped in at runtime (`... | cache-warden kv set KEY`).
...
```toml
[kv.GITHUB_KEY]                              # op source definition for the private-key PEM (no preload needed)
source = "op"
...
- **Typed source / auth schema** ([DR-0018](./decisions/DR-0018-typed-sources-auth-and-prefetch.md)):
  `source = "<kind>"` discriminator + kind-specific sub-tables (`command.*` / `op.*`) replace the bare
  `command = [...]` array form; `[auth]` gains a required `type` field. The `op` kind preserves the typed
  original in the definition (uri visible in status; idempotency compared on the typed form). A second
  opaque slot in core `Definition` holds `SourceMeta` (orthogonal to `ValueMeta`); the core stores and
  compares it but never interprets it. **Implemented**: `source = "command"` / `"op"` config + defs +
  CLI sugar (`--command ARGV...`, `--source op://URI`, `--command-cwd`, `--command-env`) /
  `[auth] type = "command"` (bare `[auth] command = [...]` without `type` is an error) /
  typed wire in `kv.define` / status shows typed original (op uri visible; command status hides argv,
  config show exposes it).
...
- [DR-0018-typed-sources-auth-and-prefetch](./decisions/DR-0018-typed-sources-auth-and-prefetch.md) — Typed source / auth schema
