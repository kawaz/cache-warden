...
- **コマンド v1**: `ping` / `status`（デーモン情報 + エントリ一覧。**値は含めない**。「定義のみ（値未生成）」のキーも列挙し defined / has_value / state を区別。pin 中は残り秒を併記）/
  ...
  `kv.get`（値不在 or HardExpired でも定義があれば lazy 生成。SoftExpired は再認証で延長、HardExpired + regenerable は再生成して値を返す）/
...
`kv define|set|get|del|list|pin|unpin` / `config show|path|edit`（引数なしは help、ロングオプション）。
