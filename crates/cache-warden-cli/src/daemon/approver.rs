...
/// Accept one connection on `listener`, send `request`, and wait for the
/// helper's `ApproveResponse` — the whole exchange (accept + send + receive)
/// bounded by `timeout`.
///
/// The bound covers `accept()` too, not just the response read: a helper that
/// crashes before connecting (bad binary path resolved late, codesign kill,
/// fail-fast exit on its side) would otherwise hang the daemon on `accept()`
/// forever, and the daemon has no other liveness signal at this layer.
///
/// Deliberately split from [`spawn_helper`]: a test can drive this function
/// directly with a fake `tokio::spawn`ed task that connects to the same
/// socket and speaks the helper's half of the protocol, without ever
/// launching the real (GUI, TouchID-prompting) helper binary — see this
/// module's tests.
#[allow(dead_code)] // Phase 1.5: wired from daemon startup once guard integration lands
...
pub async fn request_approval(
    ...
    let response = exchange(listener, request, timeout).await?;
    Ok((child, response))
}
