...
impl CacheEntry {
    ...
    ///
    /// A value carries no type metadata of its own: a value *type* (e.g. otp)
    /// lives on the key's [`Definition`](crate::Definition), so a typed value is
    /// always backed by one (DR-0016). The core treats every stored value as
    /// opaque bytes.
